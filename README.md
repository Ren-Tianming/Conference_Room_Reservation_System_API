...
```bash
git clone https://github.com/Ren-Tianming/conference-room-reservation-system.git
...
| `MAX_BOOKING_DURATION_HOURS` | 1 件の最大予約時間 | `8` |
...
SQLAlchemy モデルを定義し、Alembic マイグレーションで以下のテーブルを作成します。
...
- **可用性**: Redis 障害時は開発用のプロセス内ロックへ退避でき、本番では fail-closed 設定を選択可能
